...
public class PhieuMuonDao {
    ...
    public List<PhieuMuon> getPhieuMuonTheoNguoiDoc(String maND) throws SQLException {
        String sql = """
        SELECT
            pm.ma_phieu_muon,
            nd.ho_ten AS tenNguoiDoc,
            nd.so_dien_thoai AS sdtNguoiDoc,
            nv.ho_ten AS tenNhanVien,
            pm.ngay_muon,
            pm.ngay_tra,
            CASE 
                WHEN pm.ngay_tra IS NULL THEN 'Đang mượn' 
                ELSE 'Đã trả' 
            END AS trangThai
        FROM phieumuon pm
        JOIN nguoidoc nd ON pm.ma_nguoi_doc = nd.ma_nguoi_doc
    ...
    public PhieuMuon getById(String maPhieuMuon) throws SQLException {
        Connection conn = Database.getConnection();
    ...
    """;
        ...
        if (rs.next()) {
            p = new PhieuMuon(
                    rs.getString("ma_phieu_muon"),
                    null,  // ❗ không cần maNguoiDoc ở invoice
                    rs.getString("tenNguoiDoc"),
                    rs.getString("sdtNguoiDoc"),
                    null, // ❗ không cần maNhanVien
                    rs.getString("tenNhanVien"),
                    rs.getDate("ngay_muon"),
                    rs.getDate("ngay_tra")
            );
